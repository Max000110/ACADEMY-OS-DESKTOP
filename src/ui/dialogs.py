...
class ActivationDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Header Info
        header = QLabel("<h3>AcademyOS Software Activation</h3>")
        ...
        # Create Tabs for Online / Offline Activation
        self.tabs = QTabWidget()
        
        # Tab 1: Online Activation
        self.tab_online = QWidget()
        online_layout = QVBoxLayout(self.tab_online)
        online_layout.addWidget(QLabel("<b>Enter Customer License Key:</b>"))
        self.txt_license_key = QLineEdit()
        self.txt_license_key.setPlaceholderText("AOS-LIC-XXXXXXXX")
        online_layout.addWidget(self.txt_license_key)
        
        desc_online = QLabel("Activates the software automatically via the internet.")
        desc_online.setStyleSheet("color: gray; font-size: 11px;")
        online_layout.addWidget(desc_online)
        online_layout.addStretch()
        
        btn_online_activate = QPushButton("Activate Online")
        btn_online_activate.clicked.connect(self.activate_online)
        online_layout.addWidget(btn_online_activate)
        
        self.tabs.addTab(self.tab_online, "Online Activation")
        
        # Tab 2: Offline Activation
        self.tab_offline = QWidget()
        offline_layout = QVBoxLayout(self.tab_offline)
        offline_layout.addWidget(QLabel("<b>Enter Activation License Key:</b>"))
        self.txt_key = QLineEdit()
        self.txt_key.setPlaceholderText("AOS-KEY-YYYY-MM-DD-SIGNATURE")
        offline_layout.addWidget(self.txt_key)
        
        desc_offline = QLabel("Use the activation key generated by your administrator.")
        desc_offline.setStyleSheet("color: gray; font-size: 11px;")
        offline_layout.addWidget(desc_offline)
        offline_layout.addStretch()
        
        btn_offline_activate = QPushButton("Activate Offline")
        btn_offline_activate.clicked.connect(self.activate_offline)
        offline_layout.addWidget(btn_offline_activate)
        
        self.tabs.addTab(self.tab_offline, "Offline Activation")
    ...
    def activate_online(self):
        ...
        lic_key = self.txt_license_key.text().strip()
        if not lic_key:
            QMessageBox.warning(self, "Blank Key", "Please input your Customer License Key.")
            return
        ...
        try:
            ...
            if activate_license(activation_key):
                QMessageBox.information(self, "Success", "AcademyOS activated online successfully! The application will now load.")
                try:
                    from src.utils.config import load_settings, save_settings
                    settings = load_settings()
                    settings["license_key"] = lic_key
                    save_settings(settings)
                except Exception:
                    pass
                self.accept()
            else:
                QMessageBox.critical(self, "Activation Failed", "Failed to apply activation key locally.")
            ...
            try:
                ...
                detail = str(e)
            QMessageBox.critical(self, "Activation Failed", f"Server error: {detail}")
        except urllib.error.URLError as e:
            QMessageBox.critical(self, "Connection Error", f"Cannot connect to licensing server: {e.reason}\nPlease check your network or try offline activation.")
        except Exception as e:
