...
if grep -rnEI -e "ghp_[A-Za-z0-9_]{36}" --exclude-dir={venv,.git,build,dist,tests} . ; then
...
if grep -rnEI -e "---BEGIN [A-Z]+ PRIVATE KEY---" --exclude-dir={venv,.git,build,dist,tests} . ; then
...
if grep -rnEI -e "\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b" --exclude-dir={venv,.git,build,dist,tests} . | grep -vE "(127\.0\.0\.1|0\.0\.0\.0|192\.168\.|10\.)" ; then
...
if grep -rnEI -e "(password|secret|salt)\s*=\s*['\"][a-zA-Z0-9_\-]{8,}['\"]" --exclude-dir={venv,.git,build,dist,tests} . | grep -vE "(dev_fallback|dev_jwt)" ; then
